...
// pages/study/[chapter].vue (script setup)
...
const { reconcileFromQuestions, statsFor } = useChapterProgress()
// keep ids & progress in sync with fetched questions
watch(questions, (qs) => {
  reconcileFromQuestions(chapter, qs || [])
}, { immediate: true })

// reactive stats for header pills
const chStats = statsFor(chapter)
// usage: chStats.value.done / due / wrong / star / last / progressPct
...
// pages/study/[chapter].vue (script setup)
const { state, dueIds, makeStats, reconcile } = useLeitner(chapter, { debug: false })
